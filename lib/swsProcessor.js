...
class SwsProcessor {
    ...
    collectRequestResponseData(res) {
        ...
        if ("headers" in req) {
            ...
            for(var hdr in req.headers){
                ...
                if (hdr.match(/authorization/i))
                    rrr.http.request.headers[hdr] = "**********";
                else
    ...
    // Remove one (or more) OpenAPI PathSpecs
    removePathSpecs(pathSpecs) {
        this.apiStats.removePathSpecs(pathSpecs);
    };

    // Add one (or more) OpenAPI PathSpecs
    addPathSpecs(pathSpecs) {
        this.apiStats.addPathSpecs(pathSpecs);
    };
}
