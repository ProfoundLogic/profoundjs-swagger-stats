...
module.exports.swsEmbeddedUIMarkup = '<!DOCTYPE html><html><head><title>Profound.js Swagger Stats</title><link href="dist/css/sws.min.css" rel="stylesheet"><link rel="icon" type="image/png" href="dist/images/favicon.png"/> \
                <script>\
                    window.parent.showSwaggerStatsRoute = function(route) {\
                        var href = window.location.href;\
                        href = href.replace(window.location.search,"").replace(window.location.hash, "");\
                        href += "?method=" + encodeURI(route["method"]) + "&path=" + encodeURI(route["path"]) + "&modfile=" + encodeURI(route["modfile"]);\
                        window.location.href = href + "#sws_apiop="+route.method.toUpperCase()+","+route.path;\
                    }\
                </script>\
                </head>\
                <body>\
