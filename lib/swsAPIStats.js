...
swsAPIStats.prototype.initialize = function(swsOptions) {
    ...
    if(!swaggerSpec.paths || !swaggerSpec.paths.length) return;

    // Enumerate all paths entries
    this.addPathSpecs(swaggerSpec.paths);
};

swsAPIStats.prototype.removePathSpecs = function (pathSpecs) {
    // If nothing passed, then remove all
    if (!pathSpecs) pathSpecs = Object.keys(this.apiMatchIndex);

    if (!Array.isArray(pathSpecs))
        pathSpecs = [pathSpecs];

    for (let i = 0; i < pathSpecs.length; i++) {
        let pathSpec = pathSpecs[i];
        delete this.apiMatchIndex[pathSpec];
        // delete this.apidefs[pathSpec];
        // delete this.apidetails[pathSpec];
        // delete this.apistats[pathSpec];
    }
}

swsAPIStats.prototype.addPathSpecs = function (pathSpecs) {
    if (!pathSpecs) return;

    for (var path in pathSpecs) {
        var pathDef = pathSpecs[path];
        ...
        for(var i=0;i<operations.length;i++){
            ...
            if(op in pathDef){
                ...
                this.processParameters(null, pathDef, opDef, fullPath, opMethod);
